        ...
        case TA_TXREPORT_WAIT_SEND:
        	{
				...
				inst->msg_f.messageData[FCODE] = RTLS_DEMO_MSG_ANCH_REPORT; //message function code (specifies if message is a poll, response or other...)
				inst->psduLength = (TAG_POLL_MSG_LEN + FRAME_CRTL_AND_ADDRESS_S + FRAME_CRC);
...
//                    	uartWriteLineNoOS((char *) dataseq2); //send some data
                ...
                else if (inst->gotTO == 1) //timeout
                {
					...
					sprintf((char*)&dataseq2[0], "report confirmation...\n ");
					uartWriteLineNoOS((char *) dataseq2); //send some data
					break;
                }
#endif
